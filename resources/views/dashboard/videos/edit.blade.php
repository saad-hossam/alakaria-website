...
@section('content')
    <div class="page-header">
        <div class="page-leftheader">
            <h4 class="page-title mb-0">تعديل الفيديو</h4>
        </div>
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">تعديل معلومات الفيديو</h3>
                </div>
                <div class="card-body">
                    <form action="{{ route('videos.update', $video->id) }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control @error('title_en') is-invalid @enderror" 
                                           id="title_en" name="title_en" value="{{ old('title_en', $video->title['en'] ?? '') }}" required>
                                    @error('title_en')
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control @error('title_ar') is-invalid @enderror" 
                                           id="title_ar" name="title_ar" value="{{ old('title_ar', $video->title['ar'] ?? '') }}" required>
                                    @error('title_ar')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <select class="form-control @error('status') is-invalid @enderror" 
                                        ...
                                        <option value="active" {{ (old('status', $video->status) == 'active') ? 'selected' : '' }}>نشط</option>
                                        <option value="disabled" {{ (old('status', $video->status) == 'disabled') ? 'selected' : '' }}>معطل</option>
                                    </select>
                                    @error('status')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                        ...
                        <div class="form-group">
                            <button type="submit" class="btn btn-primary">
                                <i class="fa fa-save"></i> تحديث الفيديو
                            </button>
                            <a href="{{ route('videos.index') }}" class="btn btn-secondary">
