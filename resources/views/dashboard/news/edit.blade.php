    ...
    <div class="breadcrumb-header justify-content-between">
        ...
        <!-- @can('news-list') -->
            <div class="d-flex my-xl-auto right-content">
                <a class="btn btn-primary btn-block" href="{{ route('news.index') }}">جميع الخدمات</a>
            </div>
        <!-- @endcan -->
    </div>
    <!-- Breadcrumb -->
                                    ...
                                    @foreach (config('app.languages') as $key => $lang)
                                        <div class="tab-pane fade @if ($loop->first) show active @endif"
                                            ...
                                            <div class="form-group mt-3">
                                                ...
                                                <textarea name="{{ $key }}[name]" class="form-control editor" id="name-{{ $key }}">{{ old($key . '.name', $news->translate($key)->name ?? '') }}</textarea>
                                            </div>
                                            <div class="form-group mt-3">
                                                <label> الوصف ({{ $lang }})</label>
                                                <textarea name="{{ $key }}[description]" class="form-control editor" id="description-{{ $key }}">{{ old($key . '.description', $news->translate($key)->description ?? '') }}</textarea>
                                            </div>
                                            <div class="form-group mt-3">
                                                <label> المحتوى ({{ $lang }})</label>
                                                <textarea name="{{ $key }}[body]" class="form-control editor" id="body-{{ $key }}">{{ old($key . '.body', $news->translate($key)->body ?? '') }}</textarea>
                                            </div>
                                        </div>
                                    @endforeach
                                </div>
                        ...
                        <div class="row mt-4">
                            ...
                            <div class="col-md-6">
                                ...
                                @if ($news->image)
                                    <div class="form-group">
                                        ...
                                        <a href="{{ asset('images/newss/' . $news->image) }}" target="_blank">
                                            <img src="{{ asset('images/newss/' . $news->image) }}"
                                                 alt="news Image"
                                                 class="img-thumbnail"
                                                 width="100">
                                        </a>
                            ...
                            <div class="col-md-6">
                                ...
                                <select class="form-control select2" name="status">
                                    <option value="active" {{ $news->status == 'active' ? 'selected' : '' }}>مفعل</option>
                                    <option value="disabled" {{ $news->status == 'disabled' ? 'selected' : '' }}>غير مفعل</option>
                                </select>
                            </div>
                        </div>
